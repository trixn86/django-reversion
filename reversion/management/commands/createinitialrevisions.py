        ...
        with transaction.atomic(using=using):
            for model in self.get_models(options):
                ...
                live_objs = _safe_subquery(
                    ...
                    model._meta.pk.name,
                    Version.objects.using(using).get_for_model(model, model_db=model_db),
                ).exclude(has_version=True)
                ...
                for i in range(0, total, batch_size):
                    ...
                    for obj in objects.values():
                        with create_revision(using=using):
                            ...
                            set_comment(comment)
                            add_to_revision(obj, type=Version.TYPE_INITIAL, model_db=model_db)
